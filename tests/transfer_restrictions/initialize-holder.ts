...
describe("Initialize transfer restriction Holder", () => {
  ...
  it("fails to initialize holder by contract admin", async () => {
    ...
    const [holderPubkey] = testEnvironment.transferRestrictionsHelper.holderPDA(zeroHolderIdx);
    try {
  ...
  it("fails to initialize holder with index greater than allowed", async () => {
    ...
    try {
      ...
      assert.equal(error.errorMessage, "Invalid transfer restriction holder index");
    }
  });
  ...
  it("initializes holder by transfer admin", async () => {
    ...
    await testEnvironment.transferRestrictionsHelper.program.methods
      ...
      .signers([signer])
      .rpc({ commitment: testEnvironment.commitment });
    const holderData = await testEnvironment.transferRestrictionsHelper.holderData(holderPubkey);
    ...
    assert.equal(holderData.transferRestrictionData.toString(), testEnvironment.transferRestrictionsHelper.transferRestrictionDataPubkey.toString());
    const trData = await testEnvironment.transferRestrictionsHelper.transferRestrictionData();
  ...
  it("initializes holder by wallets admin", async () => {
    ...
    const [authorityWalletRolePubkey] = testEnvironment.accessControlHelper.walletRolePDA(signer.publicKey);
    const [holderPubkey] = testEnvironment.transferRestrictionsHelper.holderPDA(firstHolderIdx);
    await testEnvironment.transferRestrictionsHelper.program.methods
