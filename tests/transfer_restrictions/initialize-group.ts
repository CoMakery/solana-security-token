...
describe("Initialize transfer restriction Group", () => {
  ...
  it("initializes transfer restriction group by transfer admin", async () => {
    ...
    const [authorityWalletRolePubkey] = testEnvironment.accessControlHelper.walletRolePDA(signer.publicKey);
    const [groupPubkey] = testEnvironment.transferRestrictionsHelper.groupPDA(groupIdx);
    ...
    const group = await testEnvironment.transferRestrictionsHelper.groupData(groupPubkey);
    ...
    assert.equal(group.transferRestrictionData.toString(), testEnvironment.transferRestrictionsHelper.transferRestrictionDataPubkey.toString());
  });
