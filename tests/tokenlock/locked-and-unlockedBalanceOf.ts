...
describe('TokenLockup timelock balances', () => {
    ...
    it('timelock creation with immediately unlocked tokens', async () => {
        ...
        const balance = balanceOfTimelock(account, timelockAccount, Number(timelockId), nowTs);
        ...
        assert(unlockedBalanceOf(account, timelockAccount, nowTs).toNumber() === totalRecipientAmount);
    ...
    it('can return all balance types of locked and unlocked tokens in multiple release schedules', async () => {
        ...
        assert(unlockedBalanceOf(account, timelockAccount, nowTs).toNumber() === 24);
        assert(unlockedBalanceOfTimelock(account, timelockAccount, 0, nowTs).toNumber() === 8);
        assert(unlockedBalanceOfTimelock(account, timelockAccount, 1, nowTs).toNumber() === 16);

        assert(lockedBalanceOf(account, timelockAccount, nowTs).toNumber() === 276);
        assert(lockedBalanceOfTimelock(account, timelockAccount, 0, nowTs).toNumber() === 92);
        assert(lockedBalanceOfTimelock(account, timelockAccount, 1, nowTs).toNumber() === 184);
        ...
        assert(balanceOfTimelock(account, timelockAccount, 0, nowTs).toNumber() === (8 + 92));
        assert(balanceOfTimelock(account, timelockAccount, 1, nowTs).toNumber() === (16 + 184));
    });
    ...
    it('creating a timelock increases the totalSupply and transferring decreases it', async () => {
        ...
        let balanceEscrowAfterTransfer = (await testEnvironment.mintHelper.getAccount(escrowAccount)).amount;
        ...
        balanceEscrowAfterTransfer = (await testEnvironment.mintHelper.getAccount(escrowAccount)).amount;
