...
describe('TokenLockup create release schedule', () => {
    ...
    it('should be able to check if the lockup is cancelable', async () => {
        ...
        const timelockAccountData = await getTimelockAccountData(tokenlockProgram, tokenlockDataPubkey, walletB.publicKey);
        let timelock = null;
    ...
    it('funder cannot cancel a non existent timelock', async () => {
        ...
        const scheduleCount = getScheduleCount(await getTokenlockAccount(
            tokenlockProgram,
            tokenlockDataPubkey
        ));
        ...
        assert(res === 'Permission denied, address must be present in cancelableBy');
    });
