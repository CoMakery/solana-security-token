...
describe('TokenLockup check cancelables', () => {
    ...
    it('should emit an event with the correct scheduleId when the release schedule is funded and canceled', async () => {
        ...
        const timelockCount = timelockCountOf(
            await getTimelockAccountData(tokenlockProgram, tokenlockDataPubkey, walletA.publicKey),
        );
        assert(timelockCount === 2);

        const groupId = new anchor.BN(0);
        const [groupPubkey] = testEnvironment.transferRestrictionsHelper.groupPDA(groupId);
        ...
        const [transferAdminWalletRole] = testEnvironment.accessControlHelper.walletRolePDA(testEnvironment.transferAdmin.publicKey);
        ...
        assert(timelockId === 'Permission denied, address must be present in cancelableBy');
    ...
    describe('Check cancel timelock after funding with multi cancelable addresses', () => {
        ...
        beforeEach(async () => {
            ...
            const tokenlockWalletTokenAcc = await testEnvironment.mintHelper.createAssociatedTokenAccount(
        ...
        it('cancel with third canceler', async () => {
            await topUpWallet(testEnvironment.connection, tokenlockWallet.publicKey, solToLamports(1));
            const tid = await cancelTimelock(
                tokenlockProgram,
                Number(timelockId),
                tokenlockDataPubkey,
        ...
        it('cancel with non canceler reverts', async () => {
            ...
            await topUpWallet(testEnvironment.connection, canceler.publicKey, solToLamports(1));
            const cancelerTokenAcc = await testEnvironment.mintHelper.createAssociatedTokenAccount(
                canceler.publicKey,
                testEnvironment.contractAdmin
            );
            const groupId = new anchor.BN(0);
            ...
            assert(tid === 'Permission denied, address must be present in cancelableBy');
        });
        ...
        it('timelock index not change after canceling', async () => {
            ...
            assert(timelock.commencement_timestamp === timelock1.commencement_timestamp);
    ...
    describe('simple 1 month delay then 50% for 2 monthly releases', () => {
        ...
        it('should be able to check if the lockup is cancelable', async () => {
            ...
            const timelockData = await getTimelockAccountData(tokenlockProgram, tokenlockDataPubkey, walletA.publicKey);
            const timelock = timelockOf(timelockData, timelockId);
            assert(timelock.cancelableByCount === 1);
            assert(timelockData.cancelables[timelock.cancelableBy[0]].toBase58() === walletA.publicKey.toBase58());
        });
        ...
        it('0% unlocked at start and 100% cancelable', async () => {
            ...
            let account = await getTokenlockAccount(tokenlockProgram, tokenlockDataPubkey);
            let timelockAccount = await getTimelockAccountData(tokenlockProgram, tokenlockDataPubkey, walletA.publicKey);
            ...
            let balance = balanceOfTimelock(account, timelockAccount, timelockId, nowTs);
            assert(balance.toNumber() === 100);

            const balanceEscrow = (await testEnvironment.mintHelper.getAccount(escrowAccount)).amount;
            const balanceCancelar = (await testEnvironment.mintHelper.getAccount(reserveAdminTokenAccountPubkey)).amount;
            const tid = await cancelTimelock(
                tokenlockProgram,
                Number(timelockId),
                tokenlockDataPubkey,
            ...
            const balanceEscrowAfterCancelation = (await testEnvironment.mintHelper.getAccount(escrowAccount)).amount;
            const balanceCancelarAfterCancelation = (await testEnvironment.mintHelper.getAccount(reserveAdminTokenAccountPubkey)).amount;
        ...
        it('cannot cancel a non existent timelock', async () => {
            ...
            const timelockAccount = await getTimelockAccountData(tokenlockProgram, tokenlockDataPubkey, walletA.publicKey);

            const locked = lockedBalanceOf(account, timelockAccount, nowTs);
            assert(locked.toNumber() === 100);
            const unlocked = unlockedBalanceOf(account, timelockAccount, nowTs);
            assert(unlocked.toNumber() === 0);
            const balance = balanceOfTimelock(account, timelockAccount, timelockId, nowTs);
            assert(balance.toNumber() === 100);
        });
