...
pub fn initialize_transfer_rule(ctx: Context<InitializeTransferRule>, lock_until: u64) -> Result<()> {
  let transfer_rule = &mut ctx.accounts.transfer_rule;
  let transfer_restriction_data = &mut ctx.accounts.transfer_restriction_data;
  //TODO: Add check for access control that the sender is elegible to create a transfer rule
  ...
  transfer_rule.lock_until = lock_until;
